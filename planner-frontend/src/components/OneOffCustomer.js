...
const OneOffCustomer = ( {update, close} ) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await fetch('https://jdfban.pythonanywhere.com/api/add_one_off/', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          toast.success('Job saved to planner!', { position: toast.POSITION.TOP_CENTER });
  ...
  return (
    <div>
      <Card style={{border: 'none'}} className='new-custy m-auto'>
        <Card.Body>
          <h2 className='text-center mb-4 lobsterfont'>Book a non-recurring job</h2>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group id="notes">
              ...
              <Form.Control type="textarea" onChange={(e) => setNotes(e.target.value)} value={notes} />
            </Form.Group>
            <Form.Group id="email">
